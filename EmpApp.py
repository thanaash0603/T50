...
def addleave():
    ...
    insert_sql = "INSERT INTO leaveEmployee VALUES (%s, %s, %s, %s, %s, %s)"
    cursor = db_conn.cursor()
    if emp_image_file.filename == "":
       return "Please select a file"
    ...
    try:
        ...
        cursor.execute(insert_sql, (id, name, office, date, contact, team))
        db_conn.commit()
        # # Uplaod image file in S3 #
        emp_image_file_name_in_s3 = "fmno-" + str(id) + "_image_file"
        s3 = boto3.resource('s3')
...
def Getleavet():
    ...
    select_sql = "SELECT * FROM leaveEmployee WHERE id = %s"
    cursor = db_conn.cursor()
    ...
    try:
        ...
        db_conn.commit()
        print("Data fetched from MySQL RDS... fetching image from S3...")
        (id, name, office, date, contact, team) = cursor.fetchone()
    ...
    print("all modification done...")
    return render_template('GetLeaveOutput.html', id, name=name, office=office, date=date,
